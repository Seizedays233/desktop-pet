...
一只以真实橘猫为原型制作的跨平台桌面宠物。程序使用透明、无边框、置顶窗口显示小猫，支持抚摸、投喂、拖动、随机待机动画、跟随鼠标和自动散步等交互。Windows 版本还可以把文件或文件夹“喂”给小猫，由小猫播放进食动画并将它们送入系统回收站。

> 项目目前包含 Windows 原生版本，以及基于 Avalonia 的 macOS/Linux 跨平台版本。

## 🌟 新功能：把文件喂给小猫

Windows 版本现已支持将小猫作为一个有动画效果的桌面回收站：

1. 从资源管理器中选中一个或多个文件、文件夹。
2. 将它们拖到桌面上的小猫身上。
3. 看到提示后松开鼠标，小猫会播放吃东西动画。
4. 动画结束后，文件或文件夹会被移动到 Windows 系统回收站。

该功能支持同时处理多个文件和文件夹。被处理的内容进入系统回收站，并非直接永久删除；如有需要，可以在 Windows 回收站中恢复。

> **重要提示**
>
> - 回收站功能目前仅适用于 Windows WPF 版本，macOS/Linux 版本暂不支持。
> - 请以普通用户身份启动程序，不要选择“以管理员身份运行”，否则 Windows 的权限隔离可能阻止从资源管理器拖放文件。
> - 磁盘根目录、共享根目录、不存在的路径及系统不允许回收的项目不会被处理。
> - 如果一次拖入多个项目，程序会分别处理；个别项目失败不会影响已经成功移入回收站的项目。

## 功能特性

- **Windows 桌面回收站：拖入文件或文件夹，小猫播放进食动画后将其移入系统回收站**
- 透明背景、无边框桌面显示
- 默认始终置顶，可通过右键菜单关闭
- 单击抚摸，小猫会闭眼并伸头享受
- 双击打开投喂菜单
...
| 操作 | 效果 |
| --- | --- |
| 将一个或多个文件/文件夹拖到小猫身上 | 播放吃东西动画并移入 Windows 系统回收站 |
| 单击小猫 | 抚摸小猫 |
| 双击小猫 | 打开投喂菜单 |
| 按住左键拖动 | 移动小猫 |
| 鼠标滚轮 | 调整小猫大小 |
...
- 文件回收功能目前仅适配 Windows；拖放目标会进入系统回收站，不会直接永久删除，可以通过系统回收站恢复。
- 请正常启动程序，不要使用管理员模式；否则 Windows 可能阻止资源管理器向程序拖放文件。
